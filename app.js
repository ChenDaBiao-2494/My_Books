...
(function () {
  ...
  // 判断是否是相机/截图等无意义的通用文件名（这种不自动填）
  function isGenericImageName(name) {
    const n = String(name || '').toLowerCase();
    return /^(img[_\-]?\d*|dsc[_\-]?\d*|pxl[_\-]?|vid[_\-]?|image|photo|screenshot|mmexport|wx_camera|qq图片|微信图片|图片|截图)/.test(n);
  }
  ...
  function bind() {
    ...
    const onFile = (e) => {
      ...
      // 自动填书名（从封面文件名）：书名空或仍为自动填充值时填，手动改过则不动；相机通用名不填
      const t = filenameToTitle(f.name);
      if (t && !isGenericImageName(f.name) && (els.fTitle.value.trim() === '' || state.titleAutoFilled)) {
        els.fTitle.value = t;
